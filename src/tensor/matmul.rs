...
mod tests {
    ...
    fn matmul_2x2() {
        ...
        let b = Tensor::from_slice(&[2, 2], &[1.0, 0.0, 0.0, 1.0]);
        let c = a.matmul(b);
    ...
    fn matmul_output_shape() {
        let a = Tensor::from_slice(&[2, 3], &[1.0; 6]);
        let b = Tensor::from_slice(&[3, 4], &[1.0; 12]);
        let c = a.matmul(b);
    ...
    fn matmul_values() {
        let a = Tensor::from_slice(&[2, 2], &[1.0, 2.0, 3.0, 4.0]);
        let b = Tensor::from_slice(&[2, 2], &[2.0, 0.0, 1.0, 3.0]);
        let c = a.matmul(b);
